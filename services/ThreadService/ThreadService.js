...
class ThreadService {
  ...
  create(thread) {
    this._query = `INSERT INTO threads (author, forum, message, title, 
    ${thread.created ? 'created, ' : ''}slug)
    VALUES (\'${thread.author}\', \'${thread.forum}\', \'${thread.message}\', 
    \'${thread.title}\', ${thread.created ? `\'${thread.created}\',` : ''}, 
    \'${thread.slug}\');`;
  ...
  getThreadBySlug(slug, isResult = false) {
    this._query = `SELECT * FROM threads 
